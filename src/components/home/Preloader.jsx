...
const Preloader = () => {
  ...
  return (
    ...
    >
      ...
      {/* ── Skip button ── */}
      <button
        className="pl-skip-btn absolute top-4 right-4 sm:top-5 sm:right-6 z-10 flex items-center gap-1.5 group"
        onClick={handleSkip}
        aria-label="Skip intro"
        style={{ opacity: 0 }} // GSAP will reveal via autoAlpha later if needed
      >
        {/* We reveal it immediately via an inline animation so it's visible from the start */}
        <style>{`
          .pl-skip-btn {
            animation: skipFadeIn 0.4s ease 0.25s forwards;
          }
        ...
        <span
          ...
          className="group-hover:text-[#f2ede4]"
        >
          Skip
        </span>
